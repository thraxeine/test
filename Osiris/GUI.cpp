...
void GUI::renderRagebotWindow() noexcept
{
    ...
    if (&config->ragebot[currentWeapon].enabled)
    {
        ImGui::Columns(2, nullptr, false);
        ImGui::SetColumnOffset(1, 220.0f);
        ...
        ImGui::Checkbox("Fps Optimization", &config->ragebot[currentWeapon].fps_optimization);
...
void GUI::renderDefensiveWindow() noexcept
{
    ...
    ImGui::Text(" Fake Angle moved to the Anti Aim tab");
    ImGui::Text(" Soon here will be defensive");
    ImGui::NextColumn();
    ImGui::Columns(1);
}
...
void Active() { ImGuiStyle* Style = &ImGui::GetStyle(); Style->Colors[ImGuiCol_Button] = ImColor(64, 64, 64); Style->Colors[ImGuiCol_ButtonActive] = ImColor(80, 80, 80); Style->Colors[ImGuiCol_ButtonHovered] = ImColor(75, 75, 75); }
void Hovered() { ImGuiStyle* Style = &ImGui::GetStyle(); Style->Colors[ImGuiCol_Button] = ImColor(64, 64, 64); Style->Colors[ImGuiCol_ButtonActive] = ImColor(80, 80, 80); Style->Colors[ImGuiCol_ButtonHovered] = ImColor(75, 75, 75); }
        ...
        ImGui::BeginChild("##Back", ImVec2{ 704, 480 }, false);
        {
            ...
            ImGui::BeginChild("##Main", ImVec2{ 700, 470 }, false); 
            {
                ImGui::BeginChild("##UP", ImVec2{ 700, 45 }, false); 
                {
                    ...
                    ImGui::Text("manifestations");
                    ImGui::PopFont();
                    float pos = 205; // Adjusted position
                    ImGui::SetCursorPos(ImVec2{ pos, 0 });

                    if (activeTab == 2) Active(); else Hovered();
                    if (ImGui::Button("Ragebot", ImVec2{ 75, 45 })) activeTab = 2;
                    pos += 80;
                    ImGui::SetCursorPos(ImVec2{ pos, 0 });

                    if (activeTab == 3) Active(); else Hovered();
                    if (ImGui::Button("Visuals", ImVec2{ 75, 45 })) activeTab = 3;
                    pos += 80;
                    ImGui::SetCursorPos(ImVec2{ pos, 0 });

                    if (activeTab == 4) Active(); else Hovered();
                    if (ImGui::Button("Misc", ImVec2{ 75, 45 })) activeTab = 4;
                    pos += 80;
                    ImGui::SetCursorPos(ImVec2{ pos, 0 });

                    if (activeTab == 5) Active(); else Hovered();
                    if (ImGui::Button("Configs", ImVec2{ 75, 45 })) activeTab = 5;
                }
                ImGui::EndChild();
                ...
                ImGui::BeginChild("##SubTabs", ImVec2{ 700, 30 }, false); 
                {
                    ...
                    case 1: // Legitbot
                        ...
                        if (activeSubTabLegitbot == 1) Active(); else Hovered();
                        if (ImGui::Button("Main", ImVec2{ 75, 30 })) activeSubTabLegitbot = 1;
                        subTabPos += 80;
                        ImGui::SetCursorPos(ImVec2{ subTabPos, 0 });

                        if (activeSubTabLegitbot == 2) Active(); else Hovered();
                        if (ImGui::Button("Backtrack", ImVec2{ 75, 30 })) activeSubTabLegitbot = 2;
                        subTabPos += 80;
                        ImGui::SetCursorPos(ImVec2{ subTabPos, 0 });

                        if (activeSubTabLegitbot == 3) Active(); else Hovered();
                        if (ImGui::Button("Triggerbot", ImVec2{ 75, 30 })) activeSubTabLegitbot = 3;
                        subTabPos += 80;
                        ImGui::SetCursorPos(ImVec2{ subTabPos, 0 });

                        if (activeSubTabLegitbot == 4) Active(); else Hovered();
                        if (ImGui::Button("AntiAim", ImVec2{ 75, 30 })) activeSubTabLegitbot = 4;
                        break;
                    case 2: // Ragebot
                        ImGui::SetCursorPos(ImVec2{ subTabPos, 0 });
                        if (activeSubTabRagebot == 1) Active(); else Hovered();
                        if (ImGui::Button("Main", ImVec2{ 75, 30 })) activeSubTabRagebot = 1;
                        subTabPos += 80;
                        ImGui::SetCursorPos(ImVec2{ subTabPos, 0 });

                        if (activeSubTabRagebot == 2) Active(); else Hovered();
                        if (ImGui::Button("Backtrack", ImVec2{ 75, 30 })) activeSubTabRagebot = 2;
                        subTabPos += 80;
                        ImGui::SetCursorPos(ImVec2{ subTabPos, 0 });

                        if (activeSubTabRagebot == 3) Active(); else Hovered();
                        if (ImGui::Button("Anti aim", ImVec2{ 75, 30 })) activeSubTabRagebot = 3;
                        subTabPos += 80;
                        ImGui::SetCursorPos(ImVec2{ subTabPos, 0 });

                        if (activeSubTabRagebot == 4) Active(); else Hovered();
                        if (ImGui::Button("Defensive", ImVec2{ 75, 30 })) activeSubTabRagebot = 4;
                        subTabPos += 80;
                        ImGui::SetCursorPos(ImVec2{ subTabPos, 0 });
                    ...
                    case 3: // Visuals
                        ImGui::SetCursorPos(ImVec2{ subTabPos, 0 });
                        if (activeSubTabVisuals == 1) Active(); else Hovered();
                        if (ImGui::Button("Main", ImVec2{ 75, 30 })) activeSubTabVisuals = 1;
                        subTabPos += 80;
                        ImGui::SetCursorPos(ImVec2{ subTabPos, 0 });

                        if (activeSubTabVisuals == 2) Active(); else Hovered();
                        if (ImGui::Button("Esp", ImVec2{ 75, 30 })) activeSubTabVisuals = 2;
                        subTabPos += 80;
                        ImGui::SetCursorPos(ImVec2{ subTabPos, 0 });

                        if (activeSubTabVisuals == 3) Active(); else Hovered();
                        if (ImGui::Button("Chams", ImVec2{ 75, 30 })) activeSubTabVisuals = 3;
                        subTabPos += 80;
                        ImGui::SetCursorPos(ImVec2{ subTabPos, 0 });

                        if (activeSubTabVisuals == 4) Active(); else Hovered();
                        if (ImGui::Button("Glow", ImVec2{ 75, 30 })) activeSubTabVisuals = 4;
                        subTabPos += 80;
                        ImGui::SetCursorPos(ImVec2{ subTabPos, 0 });

                        if (activeSubTabVisuals == 5) Active(); else Hovered();
                        if (ImGui::Button("Skins", ImVec2{ 75, 30 })) activeSubTabVisuals = 5;
                        break;
                    case 4: // Misc
                        ImGui::SetCursorPos(ImVec2{ subTabPos, 0 });
                        if (activeSubTabMisc == 1) Active(); else Hovered();
                        if (ImGui::Button("Main", ImVec2{ 75, 30 })) activeSubTabMisc = 1;
                        subTabPos += 80;
                        ImGui::SetCursorPos(ImVec2{ subTabPos, 0 });

                        if (activeSubTabMisc == 2) Active(); else Hovered();
                        if (ImGui::Button("Sound", ImVec2{ 75, 30 })) activeSubTabMisc = 2;
                        break;
                    }
                }
                ImGui::EndChild();
